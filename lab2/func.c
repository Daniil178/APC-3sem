...
unsigned long int InvSbox[16][16] = {{
...
unsigned long int Invers(unsigned long int k0) {
    ...
    for (int i = 0; i < 32; ++i) {
        if (i < 16)
            k1 |= (k0 & ((unsigned long int) 0b1 << i)) << (31 - 2 * i);
        else
            k1 |= (k0 & ((unsigned long int) 1 << i)) >> (2 * i - 31);
    }
    return k1;
}
...
unsigned long int SubBytes(unsigned long int p, char t) {
    ...
    if (t == 'e')
        for (int i = 0; i < 4; ++i) {
            c |= Sbox[(p & ((unsigned long int) 0xf << (4 + 8 * i))) >> (4 + 8 * i)][(p & ((unsigned long int) 0xf << (8 * i))) >> (8 * i)] << (8 * i);
        }
    else
        for (int i = 0; i < 4; ++i) {
            c |= InvSbox[(p & ((unsigned long int) 0xf << (4 + 8 * i))) >> (4 + 8 * i)][(p & ((unsigned long int) 0xf << (8 * i))) >> (8 * i)] << (8 * i);
        }
    return c;
}
...
unsigned long int ShiftRows(unsigned long int p) {
    unsigned long int c = 0;
    c |= (p & (unsigned long int) 0xffff0000);
    c |= (p & (unsigned long int) 0x000000ff) << 8;
    c |= (p & (unsigned long int) 0x0000ff00) >> 8;
    return c;
}
